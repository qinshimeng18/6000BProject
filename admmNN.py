...
def UpdateZ1(a,w_a):
    def f_z(z):
        return Gamma*(a-Relu(z))**2 + Beta*(z-w_a)**2
...
vactivation = vectorize(Relu)
vUpdateZ1 = vectorize(UpdateZ1)
vUpdateZ2 = vectorize(UpdateZ2)
vprediction = vectorize(prediction)
vcomputeLoss = vectorize(computeLoss)

def updateParameters(is_warmStart = False):
    ...
    z2 = vUpdateZ1(a2, np.dot(W2, a1))

    # update last layer
    W3 = np.dot(z3, np.linalg.pinv(a2))
    z3 = vUpdateZ2(trainLabel, np.dot(W3, a2),_lambda)

    loss = vcomputeLoss(z3,trainLabel)
    if not is_warmStart:
...
def train_and_test_admm():
    ...
    for i in range(warmStart):
        loss = updateParameters(is_warmStart=True)
        print("warm start, err :{}".format(loss))
    iteration = 1
    while 1:
        loss = updateParameters(is_warmStart=False)
